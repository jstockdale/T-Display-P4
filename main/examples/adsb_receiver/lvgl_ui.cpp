...
extern "C" {
    ...
    // MSC mode wrapper functions (defined in main.cpp)
    extern bool sd_msc_enter_fn(void);
    extern void sd_msc_exit_fn(void);
    extern bool sd_msc_is_active_fn(void);
    extern void sd_msc_get_stats_fn(uint64_t *, uint64_t *);
    // Time manager wrapper (defined in main.cpp)
    extern void time_manager_set_ntp_config_fn(const char *, int32_t);
    // Haptic feedback (defined in main.cpp)
    extern void haptic_tick(void);
}
...
#define SCOPE_TRAIL_MAX   MAX_SCOPE_AIRCRAFT

struct ScopeTrailPt { double lat, lon; int16_t alt; int16_t speed; };
struct ScopeTrailAC {
    uint32_t icao;
...
// PSRAM-allocated aircraft buffers (replace three static 64-slot .bss arrays)
// g_trail_ac: used by esp_timer trail callback (separate task from LVGL)
// g_scope_ac: shared by LVGL task callsites (aircraft list update + scope radar draw)
static scope_aircraft_t *g_trail_ac = nullptr;
static scope_aircraft_t *g_scope_ac = nullptr;

// Timer callback — records aircraft positions at 1Hz
static void scope_trail_timer_cb(void *arg) {
    if (!g_trails || !g_trail_ac) return;

    int ac_count = adsb_get_aircraft_for_scope(g_trail_ac, MAX_SCOPE_AIRCRAFT);

    for (int i = 0; i < ac_count; i++) {
        if (!g_trail_ac[i].has_position) continue;
        if (g_trail_ac[i].lat == 0.0 && g_trail_ac[i].lon == 0.0) continue;

        int slot = -1;
        for (int t = 0; t < g_trail_count; t++) {
            if (g_trails[t].icao == g_trail_ac[i].icao) { slot = t; break; }
        }
        if (slot < 0 && g_trail_count < SCOPE_TRAIL_MAX) {
            slot = g_trail_count++;
            g_trails[slot].icao = g_trail_ac[i].icao;
            g_trails[slot].head = 0;
            g_trails[slot].count = 0;
        }
        if (slot >= 0) {
            g_trails[slot].pts[g_trails[slot].head] = {
                g_trail_ac[i].lat, g_trail_ac[i].lon,
                (int16_t)(g_trail_ac[i].altitude > 32767 ? 32767 : g_trail_ac[i].altitude),
                (int16_t)(g_trail_ac[i].speed > 32767 ? 32767 : g_trail_ac[i].speed)
            };
...
extern "C" void scope_trail_init(void) {
    ...
    // Allocate shared aircraft buffers in PSRAM (replaces three static .bss arrays)
    if (!g_trail_ac) {
        g_trail_ac = (scope_aircraft_t *)heap_caps_calloc(
            MAX_SCOPE_AIRCRAFT, sizeof(scope_aircraft_t), MALLOC_CAP_SPIRAM);
    }
    if (!g_scope_ac) {
        g_scope_ac = (scope_aircraft_t *)heap_caps_calloc(
            MAX_SCOPE_AIRCRAFT, sizeof(scope_aircraft_t), MALLOC_CAP_SPIRAM);
    }
    if (!g_trail_ac || !g_scope_ac) {
        printf("[TRAIL] Failed to allocate scope aircraft buffers in PSRAM\n");
        return;
    }
    printf("[TRAIL] Allocated scope aircraft buffers: 2 × %d slots (%d KB PSRAM)\n",
           MAX_SCOPE_AIRCRAFT, (int)(2 * MAX_SCOPE_AIRCRAFT * sizeof(scope_aircraft_t) / 1024));
...
#error "unknown macro definition, please select the correct macro definition."
        ...
        for (uint16_t i = 0; i < sizeof(_win_home_app_icon_list) / sizeof(Win_Home_App_Icon); i++)
        {
            ...
            // 设置按钮位置
            lv_obj_align(image_button[i], LV_ALIGN_TOP_LEFT,
                         _app_style.icon.edge_distance.width + (APP_STYLE_ICON_WIDTH_HEIGHT * i) + (_app_style.icon.icon_distance.width * i) + 400,
...
#error "unknown macro definition, please select the correct macro definition."
        ...
        // Music app button (1st position — styled button, warm gradient with music note)
        {
        ...
        // Meshy app button (2nd position — styled button, green mesh icon)
        {
            ...
            lv_obj_align(meshy_btn, LV_ALIGN_TOP_LEFT,
                         _app_style.icon.edge_distance.width + (APP_STYLE_ICON_WIDTH_HEIGHT * 1) + (_app_style.icon.icon_distance.width * 1),
                         _app_style.icon.edge_distance.height + 300);
#elif defined SCREEN_ROTATION_DIRECTION_90
            lv_obj_align(meshy_btn, LV_ALIGN_TOP_LEFT,
                         _app_style.icon.edge_distance.width + (APP_STYLE_ICON_WIDTH_HEIGHT * 1) + (_app_style.icon.icon_distance.width * 1) + 400,
                         _app_style.icon.edge_distance.height);
#endif
        ...
        // ADS-B app button (3rd position — styled button with radar icon)
        {
        ...
        // Scope app button (4th position, row 1)
        {
    ...
    void System::set_sd_status(bool mounted, bool logging, bool msc)
    {
        _sd_mounted = mounted;
        _sd_logging = logging;
        _sd_msc = msc;
    ...
    {
        if (_sd_msc) {
            // Blue — USB Mass Storage mode active
            lv_label_set_text(_registry.status_bar.sd_icon, LV_SYMBOL_USB);
            lv_obj_set_style_text_color(_registry.status_bar.sd_icon, lv_color_hex(0x4488FF), (lv_style_selector_t)LV_PART_MAIN | (lv_style_selector_t)LV_STATE_DEFAULT);
        } else if (_sd_logging) {
    ...
    void System::init_win_meshy(void)
    {
        ...
        lv_obj_add_event_cb(kb, [](lv_event_t *) { haptic_tick(); }, LV_EVENT_VALUE_CHANGED, NULL);

        // Keyboard checkmark (✓) sends the message directly
        ...
        // Floating preview bar — sits just above the keyboard, mirrors textarea content
        lv_obj_t *preview_bar = lv_obj_create(_registry.win.meshy.root);
        int32_t kb_h = h * 2 / 5;
        lv_obj_set_size(preview_bar, w, 36);
        lv_obj_align(preview_bar, LV_ALIGN_BOTTOM_MID, 0, -kb_h);
        lv_obj_set_style_bg_color(preview_bar, lv_color_hex(0x0D2818), 0);
        lv_obj_set_style_border_color(preview_bar, lv_color_hex(0x2D8C3A), 0);
        lv_obj_set_style_border_width(preview_bar, 1, 0);
        lv_obj_set_style_border_side(preview_bar, LV_BORDER_SIDE_TOP, 0);
        lv_obj_set_style_radius(preview_bar, 0, 0);
        lv_obj_set_style_pad_hor(preview_bar, 8, 0);
        lv_obj_set_style_pad_ver(preview_bar, 4, 0);
        lv_obj_set_scrollbar_mode(preview_bar, LV_SCROLLBAR_MODE_OFF);
        lv_obj_set_scroll_dir(preview_bar, LV_DIR_HOR);
        lv_obj_add_flag(preview_bar, LV_OBJ_FLAG_HIDDEN);

        lv_obj_t *preview_lbl = lv_label_create(preview_bar);
        lv_label_set_long_mode(preview_lbl, LV_LABEL_LONG_SCROLL_CIRCULAR);
        lv_obj_set_width(preview_lbl, w - 20);
        lv_label_set_text(preview_lbl, "");
        lv_obj_set_style_text_color(preview_lbl, lv_color_hex(0x00FF00), 0);
        lv_obj_set_style_text_font(preview_lbl, &lv_font_montserrat_16, 0);

        // Pack kb + preview together for show/hide + text mirror
        struct KbPreview { lv_obj_t *kb; lv_obj_t *bar; lv_obj_t *lbl; };
        KbPreview *kbp = (KbPreview *)lv_malloc(sizeof(KbPreview));
        kbp->kb = kb; kbp->bar = preview_bar; kbp->lbl = preview_lbl;

        // Show keyboard + preview on textarea focus, hide on defocus
        lv_obj_add_event_cb(_registry.win.meshy.tx_textarea, [](lv_event_t *e) {
            KbPreview *kbp = (KbPreview *)lv_event_get_user_data(e);
            lv_event_code_t code = lv_event_get_code(e);
            if (code == LV_EVENT_FOCUSED) {
                lv_keyboard_set_textarea(kbp->kb, lv_event_get_target_obj(e));
                lv_obj_remove_flag(kbp->kb, LV_OBJ_FLAG_HIDDEN);
                lv_obj_remove_flag(kbp->bar, LV_OBJ_FLAG_HIDDEN);
                // Sync preview with current textarea content
                const char *txt = lv_textarea_get_text(lv_event_get_target_obj(e));
                lv_label_set_text(kbp->lbl, txt && txt[0] ? txt : "");
            } else if (code == LV_EVENT_DEFOCUSED || code == LV_EVENT_READY) {
                lv_obj_add_flag(kbp->kb, LV_OBJ_FLAG_HIDDEN);
                lv_obj_add_flag(kbp->bar, LV_OBJ_FLAG_HIDDEN);
            } else if (code == LV_EVENT_VALUE_CHANGED) {
                // Mirror textarea content to preview on every keystroke
                const char *txt = lv_textarea_get_text(lv_event_get_target_obj(e));
                lv_label_set_text(kbp->lbl, txt && txt[0] ? txt : "");
            }
        }, LV_EVENT_ALL, kbp);
    ...
    void System::win_meshy_update(const char *stats_text, const char *msg_text)
    {
        ...
        if (_registry.win.meshy.msg_canvas && _registry.win.meshy.msg_canvas_buf) {
            ...
            int32_t line_h = 28;
            // Reserve headroom for message wrapping — some messages consume
            // 2-3 visual lines, so select fewer messages than raw line count
            // to ensure the newest messages always render within the canvas.
            int32_t max_visual_lines = (buf_h - 10) / line_h;
            int32_t max_lines = max_visual_lines * 2 / 3;  // ~42 messages, room for ~21 wraps
            ...
            // Check if user is viewing the latest content before redraw.
            // The canvas is always 1800px tall, but content only fills to y.
            // "At bottom" means the user's viewport includes the content edge —
            // either they haven't scrolled (scroll_y near 0, content fits in panel),
            // or they're scrolled near the content bottom.
            bool at_bottom = true;
            static int32_t prev_content_y = 0;  // content height from last redraw
            if (panel) {
                int32_t scroll_y = lv_obj_get_scroll_y(panel);
                int32_t panel_h = lv_obj_get_content_height(panel);
                int32_t viewport_bottom = scroll_y + panel_h;
                // User is "at bottom" if the viewport bottom is near or past
                // where content was drawn last time
                at_bottom = (viewport_bottom >= prev_content_y - 40) || (prev_content_y <= panel_h);
            }
            ...
            // Usable pixel width for text (canvas minus left+right margin)
            int32_t text_max_w = cw - 8;
            ...
            for (int i = render_start; i < render_start + render_count && y < buf_h - line_h; i++) {
                ...
                // Per-line color: TX=accent cyan, RX text=white, other=dim green
                lv_color_t line_color;
                if (strstr(line_buf, "[You]")) {
                    line_color = lv_color_hex(0x00E5A0);  // our TX — accent green
                } else if (strstr(line_buf, "] ") && !strstr(line_buf, "] @") &&
                           !strstr(line_buf, "] joined") && !strstr(line_buf, "] bat:")) {
                    line_color = lv_color_hex(0xFFFFFF);  // RX text — white
                } else {
                    line_color = lv_color_hex(0x88DDAA);  // position/telem/join — dim green
                }

                // Manual word-wrap: split long lines at word boundaries so each
                // chunk fits within canvas width. draw_text_with_emoji never wraps
                // (area width = 16384), so chunks that fit will render cleanly and
                // any slight overestimate just clips at the canvas edge.
                char *remaining = line_buf;
                while (*remaining && y < buf_h - line_h) {
                    // Check if the rest fits on one line
                    lv_point_t full_sz;
                    lv_text_get_size(&full_sz, remaining, &lv_font_montserrat_22,
                                     0, 0, 16384, LV_TEXT_FLAG_EXPAND);
                    if (full_sz.x <= text_max_w) {
                        // Fits — draw and done with this message
                        draw_text_with_emoji((uint16_t *)_registry.win.meshy.msg_canvas_buf,
                                             cw, buf_h, &layer,
                                             remaining, 4, y,
                                             &lv_font_montserrat_22, line_color);
                        y += line_h;
                        break;
                    }

                    // Doesn't fit — find the longest prefix that does.
                    // Walk down from full length (already know it doesn't fit).
                    int rlen = strlen(remaining);
                    int cut = 1;  // fallback: at least 1 char per chunk to guarantee progress

                    static char measure_buf[512];
                    for (int try_len = rlen - 1; try_len > 0; try_len--) {
                        if (try_len > (int)sizeof(measure_buf) - 1) continue;
                        memcpy(measure_buf, remaining, try_len);
                        measure_buf[try_len] = '\0';
                        lv_point_t sz;
                        lv_text_get_size(&sz, measure_buf, &lv_font_montserrat_22,
                                         0, 0, 16384, LV_TEXT_FLAG_EXPAND);
                        if (sz.x <= text_max_w) {
                            cut = try_len;
                            break;
                    ...
                    // Back up to a word boundary (space)
                    int break_at = cut;
                    for (int j = cut; j > cut / 2; j--) {
                        if (remaining[j] == ' ') {
                            break_at = j;
                            break;
                        }
                    ...
                    // Draw this chunk
                    char saved = remaining[break_at];
                    remaining[break_at] = '\0';
                    draw_text_with_emoji((uint16_t *)_registry.win.meshy.msg_canvas_buf,
                                         cw, buf_h, &layer,
                                         remaining, 4, y,
                                         &lv_font_montserrat_22, line_color);
                    remaining[break_at] = saved;
                    y += line_h;

                    // Advance past the break — skip the space if we broke on one
                    remaining += break_at;
            ...
            prev_content_y = y;
            if (panel && at_bottom && y > 0) {
                int32_t panel_h = lv_obj_get_content_height(panel);
                if (y > panel_h) {
                    // Content exceeds panel — scroll to show bottom
                    int32_t target_y = y - panel_h + 20;
                    lv_obj_scroll_to_y(panel, target_y, LV_ANIM_OFF);
                } else {
                    // Content fits in panel — stay at top
                ...
                if (s.touch_active) {
                    ...
                    if (dx * dx + dy * dy < 400) {  // ~20px movement threshold
                        ...
                        if (same_area && s.tap_candidate_count > 1) {
                            ...
                            // Build new candidate list — uses shared PSRAM buffer
                            int ac_cnt = g_scope_ac ? adsb_get_aircraft_for_scope(g_scope_ac, MAX_SCOPE_AIRCRAFT) : 0;
                            ...
                            if (!has_fix) {
                                ...
                                for (int i = 0; i < ac_cnt; i++) {
                                    if (g_scope_ac[i].has_position) { sl += g_scope_ac[i].lat; sn += g_scope_ac[i].lon; np++; }
                                }
                            ...
                            for (int i = 0; i < ac_cnt && ncands < 8; i++) {
                                if (!g_scope_ac[i].has_position) continue;
                                int32_t ax, ay;
                                if (has_fix) {
                                    double brd = g_scope_ac[i].bearing_deg * M_PI / 180.0;
                                    ax = cx + (int32_t)(g_scope_ac[i].dist_nm * sin(brd) * scale + s.pan_x);
                                    ay = cy + (int32_t)(-g_scope_ac[i].dist_nm * cos(brd) * scale + s.pan_y);
                                } else {
                                    double ddx = (g_scope_ac[i].lon - center_lon) * 60.0 * cos_lat;
                                    double ddy = -(g_scope_ac[i].lat - center_lat) * 60.0;
                                ...
                                if (dsq < 3600) { // 60px radius
                                    cands[ncands++] = { g_scope_ac[i].icao, dsq };
                                }
                            }
                            // Sort by distance
    ...
    void System::win_scope_redraw(void)
    {
        ...
        static char ac_labels[MAX_SCOPE_AIRCRAFT][28];
        ...
        int max_ac = MAX_SCOPE_AIRCRAFT;
        if (g_settings.scope_max_aircraft > 0 && g_settings.scope_max_aircraft < MAX_SCOPE_AIRCRAFT)
            max_ac = g_settings.scope_max_aircraft;
        int ac_count = g_scope_ac ? adsb_get_aircraft_for_scope(g_scope_ac, max_ac) : 0;
        receiver_pos_t rx = adsb_get_receiver_pos();

        // Count positioned aircraft
        int positioned = 0;
        for (int i = 0; i < ac_count; i++) {
            if (g_scope_ac[i].has_position) positioned++;
        }
        ...
        if (!has_fix && positioned > 0) {
            ...
            for (int i = 0; i < ac_count; i++) {
                if (g_scope_ac[i].has_position) {
                    sum_lat += g_scope_ac[i].lat;
                    sum_lon += g_scope_ac[i].lon;
                }
            }
        ...
        static double ac_dx[MAX_SCOPE_AIRCRAFT], ac_dy[MAX_SCOPE_AIRCRAFT], ac_dist[MAX_SCOPE_AIRCRAFT];
        ...
        for (int i = 0; i < ac_count; i++) {
            ...
            if (has_fix) {
                ...
                ac_dx[i] = g_scope_ac[i].dist_nm * sin(g_scope_ac[i].bearing_deg * M_PI / 180.0);
                ac_dy[i] = -g_scope_ac[i].dist_nm * cos(g_scope_ac[i].bearing_deg * M_PI / 180.0);  // -Y = north
                ac_dist[i] = g_scope_ac[i].dist_nm;
            } else {
                // Equirectangular from centroid
                ac_dx[i] = (g_scope_ac[i].lon - center_lon) * nm_per_deg_lon;
                ac_dy[i] = -(g_scope_ac[i].lat - center_lat) * nm_per_deg_lat;  // -Y = north
        ...
        if (g_trails) {
          ...
          for (int t = 0; t < g_trail_count; t++) {
              ...
              for (int a = 0; a < ac_count; a++) {
                  if (g_scope_ac[a].icao == g_trails[t].icao) { t_age = g_scope_ac[a].age_ms; break; }
              }
          ...
          for (int t = 0; t < g_trail_count; t++) {
            ...
            for (int a = 0; a < ac_count; a++) {
                if (g_scope_ac[a].icao == g_trails[t].icao) {
                    trail_alt = g_scope_ac[a].altitude; trail_spd = g_scope_ac[a].speed;
                    trail_age_ms = g_scope_ac[a].age_ms; break;
                }
            }
            ...
            int cm = _registry.win.scope.color_mode;
            // Per-segment coloring in ALT/SPD modes — use each point's recorded values
            bool per_seg = !trail_gray && (cm == 2 || cm == 3);
            lv_color_t trail_ac_color = trail_gray ? lv_color_hex(0x556677) :
                scope_aircraft_color(cm, g_trails[t].icao, trail_alt, trail_spd);
            if (trail_sel && cm == 0 && !trail_gray)
            ...
            for (int j = 0; j < total; j += step) {
                ...
                if (prev_x >= 0) {
                    ...
                    tdsc.color = per_seg ?
                        scope_aircraft_color(cm, g_trails[t].icao,
                            g_trails[t].pts[idx].alt, g_trails[t].pts[idx].speed)
                        : trail_ac_color;
            ...
            if (total > 1 && step > 1) {
                ...
                if (prev_x >= 0 && tx >= 0 && tx <= cw && ty >= 0 && ty <= ch) {
                    ...
                    tdsc.color = per_seg ?
                        scope_aircraft_color(cm, g_trails[t].icao,
                            g_trails[t].pts[last_idx].alt, g_trails[t].pts[last_idx].speed)
                        : trail_ac_color;
                    tdsc.width = trail_width;
                    tdsc.opa = 220;
                    tdsc.p1.x = prev_x; tdsc.p1.y = prev_y;
        ...
        for (int i = 0; i < ac_count && drawn < MAX_SCOPE_AIRCRAFT; i++) {
            if (!g_scope_ac[i].has_position) continue;

            bool selected = (g_scope_ac[i].icao == _registry.win.scope.selected_icao);
            ...
            int32_t age_s = g_scope_ac[i].age_ms / 1000;
            uint8_t age_opa;
            bool age_gray;
            ...
            lv_color_t ac_color = age_gray ? col_gray :
                scope_aircraft_color(_registry.win.scope.color_mode,
                                     g_scope_ac[i].icao, g_scope_ac[i].altitude, g_scope_ac[i].speed);
            ...
            // Check aircraft class for helicopter icon
            aircraft_db_entry_t ac_db;
            bool is_heli = aircraft_db_lookup(g_scope_ac[i].icao, &ac_db) && ac_db.ac_class == 'H';

            if (is_heli) {
                // ── Helicopter silhouette ──
                // Design A: body (dot) + tail boom + tail rotor + X rotor blades
                // Faces east at heading 0, matching the webapp SVG orientation.
                // Local coords (centered, nose/body at +lx, tail at -lx, right side at +ly):
                //   Body:       dot at (0,0)
                //   Tail boom:  (-3,0) → (-9,0)
                //   Tail rotor: (-9,-3) → (-9,3)
                //   Rotor 1:    (8,-6.5) → (-4,6.5)
                //   Rotor 2:    (8,6.5) → (-4,-6.5)
                // Scale factor: sz/12 where sz = half-extent in pixels

                int32_t sz = selected ? 11 : 7;
                double hdg_rad = g_scope_ac[i].heading * M_PI / 180.0;
                double cs = cos(hdg_rad), sn = sin(hdg_rad);

                // Rotate local (lx,ly) to screen pixel using heading convention:
                // +lx (nose/forward) → (sin θ, -cos θ), +ly (right side) → (cos θ, sin θ)
                #define HRX(lx, ly) (ax + (int32_t)(((double)(lx)*sn + (double)(ly)*cs) * sz / 12.0))
                #define HRY(lx, ly) (ay + (int32_t)((-(double)(lx)*cs + (double)(ly)*sn) * sz / 12.0))

                // Body dot
                lv_draw_arc_dsc_t bd;
                lv_draw_arc_dsc_init(&bd);
                bd.color = dot_color;
                bd.width = selected ? 4 : 3;
                bd.center.x = ax; bd.center.y = ay;
                bd.radius = selected ? 4 : 3;
                bd.start_angle = 0; bd.end_angle = 360;
                bd.opa = age_opa;
                lv_draw_arc(&layer, &bd);

                // Tail boom: extends behind body (-lx direction)
                lv_draw_line_dsc_t tl;
                lv_draw_line_dsc_init(&tl);
                tl.color = dot_color; tl.opa = age_opa;
                tl.width = selected ? 2 : 1;
                tl.p1.x = HRX(-3,0);  tl.p1.y = HRY(-3,0);
                tl.p2.x = HRX(-9,0);  tl.p2.y = HRY(-9,0);
                lv_draw_line(&layer, &tl);

                // Tail rotor: perpendicular at end of boom
                tl.width = selected ? 2 : 1;
                tl.p1.x = HRX(-9,-3); tl.p1.y = HRY(-9,-3);
                tl.p2.x = HRX(-9,3);  tl.p2.y = HRY(-9,3);
                lv_draw_line(&layer, &tl);

                // Rotor blade 1 (semi-transparent)
                tl.opa = (uint8_t)(age_opa * 7 / 10);
                tl.width = 1;
                tl.p1.x = HRX(8,-6.5); tl.p1.y = HRY(8,-6.5);
                tl.p2.x = HRX(-4,6.5); tl.p2.y = HRY(-4,6.5);
                lv_draw_line(&layer, &tl);
                ...
                if (selected) {
                    ...
                    sr.color = lv_color_white();
                    sr.width = 2;
                    sr.center.x = ax; sr.center.y = ay;
                    sr.radius = 14;
                    sr.start_angle = 0; sr.end_angle = 360;
                    sr.opa = 220;
                    lv_draw_arc(&layer, &sr);
                }
            } else {
            // ── Fixed-wing icon (dot + heading line) ──
            ...
            if (g_scope_ac[i].callsign[0]) {
                snprintf(ac_labels[drawn], sizeof(ac_labels[drawn]), "%s FL%d",
                         g_scope_ac[i].callsign, g_scope_ac[i].altitude / 100);
            } else {
                snprintf(ac_labels[drawn], sizeof(ac_labels[drawn]), "%06lX FL%d",
                         (unsigned long)g_scope_ac[i].icao, g_scope_ac[i].altitude / 100);
            }
        ...
        if (_registry.win.scope.detail_label) {
            if (_registry.win.scope.selected_icao != 0) {
                for (int i = 0; i < ac_count; i++) {
                    if (g_scope_ac[i].icao == _registry.win.scope.selected_icao) {
                        bool det_gray = (g_scope_ac[i].age_ms > 60000);
                        ...
                        p += snprintf(det + p, sizeof(det) - p, "%s  %06lX  %lu msgs",
                                     g_scope_ac[i].callsign[0] ? g_scope_ac[i].callsign : "----",
                                     (unsigned long)g_scope_ac[i].icao,
                                     (unsigned long)g_scope_ac[i].msg_count);
                        // Line 2: ALT SPD HDG V/S
                        p += snprintf(det + p, sizeof(det) - p, "\nALT %d  SPD %d  HDG %d\xC2\xB0  V/S %+d",
                                     g_scope_ac[i].altitude, g_scope_ac[i].speed, g_scope_ac[i].heading, g_scope_ac[i].vert_rate);
                        // Line 3: distance + bearing + age
                        if (has_fix && g_scope_ac[i].has_position) {
                            p += snprintf(det + p, sizeof(det) - p, "\n%.1fnm  %d\xC2\xB0  %ds ago",
                                         g_scope_ac[i].dist_nm, (int)g_scope_ac[i].bearing_deg, (int)(g_scope_ac[i].age_ms / 1000));
                        } else {
                            p += snprintf(det + p, sizeof(det) - p, "\n%ds ago%s",
                                         (int)(g_scope_ac[i].age_ms / 1000), has_fix ? "" : "  (no GPS)");
                        }
                        // Lines 4–5: aircraft DB info (registration, type, model, owner)
                        {
                            aircraft_db_entry_t db;
                            if (aircraft_db_lookup(g_scope_ac[i].icao, &db)) {
                                bool show_reg = db.reg[0] &&
                                    strncmp(db.reg, g_scope_ac[i].callsign, sizeof(db.reg)) != 0;
                                p += snprintf(det + p, sizeof(det) - p, "\n%s%s%s%s%s",
                                             show_reg ? db.reg : "",
                                             show_reg && db.typecode[0] ? "  " : "",
                                             db.typecode[0] ? db.typecode : "",
                                             db.model[0] ? "  " : "",
                                             db.model[0] ? db.model : "");
                                if (db.owner[0]) {
                                    p += snprintf(det + p, sizeof(det) - p, "\n%s", db.owner);
    ...
    void System::init_win_settings(void)
    {
        ...
        // RTL-SDR dongle model (read-only, populated from adsb_get_stats)
        {
            lv_obj_t *row = add_row();
            add_row_label(row, "Dongle");
            adsb_stats_t st = adsb_get_stats();
            lv_obj_t *val = lv_label_create(row);
            lv_label_set_text(val, st.dongle_model[0] ? st.dongle_model : "not connected");
            lv_obj_set_style_text_color(val, col_value, (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_set_style_text_font(val, &lv_font_montserrat_16, (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_align(val, LV_ALIGN_RIGHT_MID, 0, 0);
        }
        ...
        // Auto Gain toggle + manual gain slider
        {
            // R820T gain steps in tenths of dB (must match class_driver.c)
            static const uint16_t r820t_gains[] = {
                0, 9, 14, 27, 37, 77, 87, 125, 144, 157,
                166, 197, 207, 229, 254, 280, 297, 328, 338,
                364, 372, 386, 402, 421, 434, 439, 445, 480, 496
            };
            static const int r820t_count = sizeof(r820t_gains) / sizeof(r820t_gains[0]);

            // Find current gain index
            static int cur_idx = r820t_count - 1;
            for (int i = 0; i < r820t_count; i++) {
                if (r820t_gains[i] == g_settings.adsb_gain_tenths) { cur_idx = i; break; }
            }

            // Auto Gain toggle row
            lv_obj_t *row_auto = add_row();
            add_row_label(row_auto, "Auto Gain");
            lv_obj_t *auto_sw = lv_switch_create(row_auto);
            lv_obj_set_size(auto_sw, 56, 30);
            lv_obj_align(auto_sw, LV_ALIGN_RIGHT_MID, 0, 0);
            lv_obj_set_style_bg_color(auto_sw, lv_color_hex(0x2a3a46), (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_set_style_bg_color(auto_sw, lv_color_hex(0x00e5a0), (lv_style_selector_t)(LV_PART_INDICATOR | LV_STATE_CHECKED));
            if (g_settings.adsb_gain_mode == 0) lv_obj_add_state(auto_sw, LV_STATE_CHECKED);

            // Manual gain row (slider + value label)
            lv_obj_t *row_gain = add_row();
            add_row_label(row_gain, "Tuner Gain");

            // Value label on right showing current dB
            lv_obj_t *gain_lbl = lv_label_create(row_gain);
            char gain_txt[16];
            snprintf(gain_txt, sizeof(gain_txt), "%.1f dB", r820t_gains[cur_idx] / 10.0);
            lv_label_set_text(gain_lbl, gain_txt);
            lv_obj_set_style_text_color(gain_lbl, col_value, (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_set_style_text_font(gain_lbl, &lv_font_montserrat_16, (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_align(gain_lbl, LV_ALIGN_RIGHT_MID, 0, 0);

            // Slider (index 0–28, maps to R820T gain steps)
            lv_obj_t *gain_slider = lv_slider_create(row_gain);
            lv_obj_set_size(gain_slider, 140, 20);
            lv_obj_align(gain_slider, LV_ALIGN_RIGHT_MID, -70, 0);
            lv_slider_set_range(gain_slider, 0, r820t_count - 1);
            lv_slider_set_value(gain_slider, cur_idx, LV_ANIM_OFF);
            lv_obj_set_style_bg_color(gain_slider, lv_color_hex(0x2a3a46), (lv_style_selector_t)LV_PART_MAIN);
            lv_obj_set_style_bg_color(gain_slider, lv_color_hex(0x00e5a0), (lv_style_selector_t)LV_PART_INDICATOR);
            lv_obj_set_style_bg_color(gain_slider, lv_color_hex(0xc8d6e5), (lv_style_selector_t)LV_PART_KNOB);

            // Store gain label pointer in slider user_data for live update
            lv_obj_set_user_data(gain_slider, (void *)gain_lbl);

            // Slider value change — update label and apply gain live
            lv_obj_add_event_cb(gain_slider, [](lv_event_t *e) {
                if (lv_event_get_code(e) != LV_EVENT_VALUE_CHANGED) return;
                ...
                if (lbl) {
                    char txt[16];
                    snprintf(txt, sizeof(txt), "%.1f dB", gains[idx] / 10.0);
                    lv_label_set_text(lbl, txt);
                }
                // Apply gain immediately
                adsb_set_gain(1, gains[idx]);
            }, LV_EVENT_ALL, nullptr);
            ...
            // Hide gain row when auto gain is on
            if (g_settings.adsb_gain_mode == 0) {
                lv_obj_add_flag(row_gain, LV_OBJ_FLAG_HIDDEN);
            }

            // Pack references for the auto toggle callback
            struct gain_ui_ctx {
                lv_obj_t *row_gain;
                lv_obj_t *gain_slider;
                lv_obj_t *gain_lbl;
            };
            static gain_ui_ctx g_ctx;
            g_ctx.row_gain = row_gain;
            g_ctx.gain_slider = gain_slider;
            g_ctx.gain_lbl = gain_lbl;

            lv_obj_set_user_data(auto_sw, (void *)&g_ctx);
            lv_obj_add_event_cb(auto_sw, [](lv_event_t *e) {
                if (lv_event_get_code(e) != LV_EVENT_VALUE_CHANGED) return;
                lv_obj_t *sw = lv_event_get_target_obj(e);
                bool auto_on = lv_obj_has_state(sw, LV_STATE_CHECKED);
                gain_ui_ctx *ctx = (gain_ui_ctx *)lv_obj_get_user_data(sw);

                if (auto_on) {
                    // Switch to adaptive — hide slider, start at max
                    adsb_set_gain(0, 0);
                    lv_obj_add_flag(ctx->row_gain, LV_OBJ_FLAG_HIDDEN);
                } else {
                    // Switch to manual — show slider, apply current slider value
                    static const uint16_t gains[] = {
                        0, 9, 14, 27, 37, 77, 87, 125, 144, 157,
                        166, 197, 207, 229, 254, 280, 297, 328, 338,
                        364, 372, 386, 402, 421, 434, 439, 445, 480, 496
                    };
                    int idx = lv_slider_get_value(ctx->gain_slider);
                    if (idx < 0) idx = 0;
                    if (idx > 28) idx = 28;
                    adsb_set_gain(1, gains[idx]);
                    lv_obj_remove_flag(ctx->row_gain, LV_OBJ_FLAG_HIDDEN);
                }
                settings_save();
            }, LV_EVENT_ALL, nullptr);
        }
        ...
        // WIFI
        // ═══════════════════════════════════════════════════════════════
        add_section(LV_SYMBOL_WIFI "  WiFi");

        // WiFi enable toggle
        {
            lv_obj_t *row = add_row();
            add_row_label(row, "WiFi");
            add_toggle(row, &g_settings.wifi_enabled);
        }

        // SSID
        {
            lv_obj_t *row = add_row();
            add_row_label(row, "SSID");
            const char *ssid_display = g_settings.wifi_ssid[0] ? g_settings.wifi_ssid : "(not set)";
            lv_obj_t *btn = add_value_btn(row, ssid_display);
            // Get the label inside the button for later update
            lv_obj_t *btn_lbl = lv_obj_get_child(btn, 0);
            lv_obj_add_event_cb(btn, [](lv_event_t *e) {
                lv_obj_t *btn_lbl = (lv_obj_t *)lv_event_get_user_data(e);
                auto open_fn = [](lv_obj_t *parent, char *buf, size_t sz,
                                   const char *t, const char *p, lv_obj_t *bl, bool pw) {
                    // Semi-transparent scrim behind keyboard
                    ...
                    // Bottom-anchored keyboard panel (50% height)
                    lv_obj_t *overlay = lv_obj_create(scrim);
                    lv_obj_set_size(overlay, LV_PCT(100), LV_PCT(50));
                    ...
                    lv_textarea_set_max_length(ta, (uint32_t)(sz - 1));
                    lv_textarea_set_placeholder_text(ta, p);
                    if (pw) lv_textarea_set_password_mode(ta, true);
                    if (buf[0]) lv_textarea_set_text(ta, buf);
                    lv_obj_set_style_text_font(ta, &lv_font_montserrat_16, (lv_style_selector_t)LV_PART_MAIN);
                    struct Ctx { char *buf; size_t sz; lv_obj_t *bl; lv_obj_t *ov; bool pw; };
                    Ctx *ctx = (Ctx *)lv_malloc(sizeof(Ctx));
                    *ctx = { buf, sz, bl, scrim, pw };
                    lv_obj_t *kb = lv_keyboard_create(overlay);
                    lv_obj_set_width(kb, LV_PCT(100));
                    lv_obj_set_flex_grow(kb, 1);
                    lv_keyboard_set_textarea(kb, ta);
                    lv_obj_add_event_cb(kb, [](lv_event_t *) { haptic_tick(); }, LV_EVENT_VALUE_CHANGED, NULL);
                    lv_obj_add_event_cb(kb, [](lv_event_t *e2) {
                        auto code = lv_event_get_code(e2);
                        Ctx *c = (Ctx *)lv_event_get_user_data(e2);
                        if (code == LV_EVENT_READY) {
                            lv_obj_t *ta2 = lv_keyboard_get_textarea(lv_event_get_target_obj(e2));
                            const char *txt = lv_textarea_get_text(ta2);
                            strncpy(c->buf, txt ? txt : "", c->sz - 1);
                            c->buf[c->sz - 1] = '\0';
                            g_settings_save_pending = true;
                            if (c->bl) {
                                if (c->pw && c->buf[0]) lv_label_set_text(c->bl, "********");
                                else lv_label_set_text(c->bl, c->buf[0] ? c->buf : "(not set)");
                            }
                        }
                ...
                open_fn(NULL, g_settings.wifi_ssid, sizeof(g_settings.wifi_ssid),
                        "Enter WiFi SSID", "Network name", btn_lbl, false);
            }, LV_EVENT_CLICKED, btn_lbl);
        }

        // Password
        {
            lv_obj_t *row = add_row();
            add_row_label(row, "Password");
            const char *pass_display = g_settings.wifi_pass[0] ? "********" : "(not set)";
            lv_obj_t *btn = add_value_btn(row, pass_display);
            lv_obj_t *btn_lbl = lv_obj_get_child(btn, 0);
            lv_obj_add_event_cb(btn, [](lv_event_t *e) {
                lv_obj_t *btn_lbl = (lv_obj_t *)lv_event_get_user_data(e);
                auto open_fn = [](char *buf, size_t sz, const char *t, const char *p, lv_obj_t *bl, bool pw) {
                    // Semi-transparent scrim behind keyboard
                    lv_obj_t *scrim = lv_obj_create(lv_screen_active());
                    lv_obj_set_size(scrim, LV_PCT(100), LV_PCT(100));
                    lv_obj_set_style_bg_color(scrim, lv_color_black(), 0);
                ...
                open_fn(g_settings.wifi_pass, sizeof(g_settings.wifi_pass),
                        "Enter WiFi Password", "Password", btn_lbl, true);
            }, LV_EVENT_CLICKED, btn_lbl);
        }
        ...
        {
            lv_obj_t *row = add_row();
            add_row_label(row, "NTP Server");
            lv_obj_t *btn = add_value_btn(row, g_settings.ntp_server);
            lv_obj_t *btn_lbl = lv_obj_get_child(btn, 0);
            lv_obj_add_event_cb(btn, [](lv_event_t *e) {
                lv_obj_t *btn_lbl = (lv_obj_t *)lv_event_get_user_data(e);
                auto open_fn = [](char *buf, size_t sz, const char *t, const char *p, lv_obj_t *bl) {
                    // Semi-transparent scrim behind keyboard
                    lv_obj_t *scrim = lv_obj_create(lv_screen_active());
                    lv_obj_set_size(scrim, LV_PCT(100), LV_PCT(100));
                    lv_obj_set_style_bg_color(scrim, lv_color_black(), 0);
                    ...
                    if (buf[0]) lv_textarea_set_text(ta, buf);
                    lv_obj_set_style_text_font(ta, &lv_font_montserrat_16, (lv_style_selector_t)LV_PART_MAIN);
                    struct Ctx { char *buf; size_t sz; lv_obj_t *bl; lv_obj_t *ov; };
                    Ctx *ctx = (Ctx *)lv_malloc(sizeof(Ctx));
                    *ctx = { buf, sz, bl, scrim };
                    lv_obj_t *kb = lv_keyboard_create(overlay);
                    lv_obj_set_width(kb, LV_PCT(100));
                    ...
                    lv_obj_add_event_cb(kb, [](lv_event_t *e2) {
                        ...
                        if (code == LV_EVENT_READY) {
                            ...
                            c->buf[c->sz - 1] = '\0';
                            g_settings_save_pending = true;
                            if (c->bl) lv_label_set_text(c->bl, c->buf[0] ? c->buf : "(not set)");
                            time_manager_set_ntp_config_fn(c->buf, g_settings.ntp_poll_s);
                        }
                        if (code == LV_EVENT_READY || code == LV_EVENT_CANCEL) {
                            lv_obj_t *ov = c->ov;
                            lv_free(c);
                ...
                open_fn(g_settings.ntp_server, sizeof(g_settings.ntp_server),
                        "Enter NTP Server", "pool.ntp.org", btn_lbl);
            }, LV_EVENT_CLICKED, btn_lbl);
        }
        ...
        // USB STORAGE
        // ═══════════════════════════════════════════════════════════════
        add_section(LV_SYMBOL_USB "  USB Storage");

        {
            lv_obj_t *row = add_row();
            add_row_label(row, "USB Mass Storage");
            lv_obj_t *btn = add_value_btn(row, "CONNECT");
            auto *self = this;
            lv_obj_add_event_cb(btn, [](lv_event_t *e) {
                auto *self = (Lvgl_Ui::System *)lv_event_get_user_data(e);
                ...
                lv_label_set_text(title, "Enter USB Storage Mode?");
                lv_obj_set_style_text_font(title, &lv_font_montserrat_16, 0);

                lv_obj_t *msg = lv_label_create(dialog);
                lv_label_set_text(msg, "SD logging and ADS-B receiver\nwill pause during USB mode.");
                ...
                lv_obj_add_event_cb(cancel, [](lv_event_t *e2) {
                    // Walk up to overlay and delete it
                    lv_obj_t *btn_row2 = lv_obj_get_parent(lv_event_get_target(e2));
                    lv_obj_t *dialog2 = lv_obj_get_parent(btn_row2);
                    lv_obj_t *overlay2 = lv_obj_get_parent(dialog2);
                    lv_obj_del(overlay2);
                }, LV_EVENT_CLICKED, NULL);

                // Confirm button
                lv_obj_t *confirm = lv_btn_create(btn_row);
                lv_obj_set_style_bg_color(confirm, lv_palette_main(LV_PALETTE_BLUE), 0);
                lv_obj_t *confirm_lbl = lv_label_create(confirm);
                lv_label_set_text(confirm_lbl, "Connect");
                lv_obj_add_event_cb(confirm, [](lv_event_t *e2) {
                    // Delete dialog
                    lv_obj_t *btn_row2 = lv_obj_get_parent(lv_event_get_target(e2));
                    lv_obj_t *dialog2 = lv_obj_get_parent(btn_row2);
                    lv_obj_t *overlay2 = lv_obj_get_parent(dialog2);
                    ...
                    // Enter MSC mode (functions declared at file scope)

                    if (sd_msc_enter_fn()) {
                        printf("[MSC] USB Storage mode active from Settings UI\n");

                        // ── MSC Status Screen ──
                        lv_obj_t *msc_screen = lv_obj_create(lv_screen_active());
                        lv_obj_set_size(msc_screen, LV_PCT(100), LV_PCT(100));
                        lv_obj_set_style_bg_color(msc_screen, lv_color_hex(0x0A0A1A), 0);
                        lv_obj_set_style_bg_opa(msc_screen, LV_OPA_COVER, 0);
                        lv_obj_set_style_border_width(msc_screen, 0, 0);
                        lv_obj_set_style_radius(msc_screen, 0, 0);
                        lv_obj_set_style_pad_all(msc_screen, 0, 0);
                        lv_obj_set_flex_flow(msc_screen, LV_FLEX_FLOW_COLUMN);
                        lv_obj_set_flex_align(msc_screen, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

                        // Branding: "Off by One"
                        lv_obj_t *brand1 = lv_label_create(msc_screen);
                        lv_label_set_text(brand1, "Off by One");
                        lv_obj_set_style_text_color(brand1, lv_color_hex(0x556677), 0);
                        lv_obj_set_style_text_font(brand1, &lv_font_montserrat_14, 0);

                        // Branding: "ADS-B Scope"
                        lv_obj_t *brand2 = lv_label_create(msc_screen);
                        lv_label_set_text(brand2, "ADS-B Scope");
                        lv_obj_set_style_text_color(brand2, lv_color_hex(0x8899BB), 0);
                        lv_obj_set_style_text_font(brand2, &lv_font_montserrat_16, 0);

                        // Spacer
                        lv_obj_t *spacer1 = lv_obj_create(msc_screen);
                        lv_obj_set_size(spacer1, 1, 16);
                        lv_obj_set_style_bg_opa(spacer1, LV_OPA_TRANSP, 0);
                        lv_obj_set_style_border_width(spacer1, 0, 0);

                        // USB Icon (large)
                        lv_obj_t *usb_icon = lv_label_create(msc_screen);
                        lv_label_set_text(usb_icon, LV_SYMBOL_USB);
                        lv_obj_set_style_text_font(usb_icon, &lv_font_montserrat_16, 0);
                        lv_obj_set_style_text_color(usb_icon, lv_color_hex(0x44AAFF), 0);

                        // "USB Storage Active"
                        lv_obj_t *status_lbl = lv_label_create(msc_screen);
                        lv_label_set_text(status_lbl, "USB Storage Active");
                        lv_obj_set_style_text_color(status_lbl, lv_color_white(), 0);
                        lv_obj_set_style_text_font(status_lbl, &lv_font_montserrat_16, 0);

                        // Spacer
                        lv_obj_t *spacer2 = lv_obj_create(msc_screen);
                        lv_obj_set_size(spacer2, 1, 8);
                        lv_obj_set_style_bg_opa(spacer2, LV_OPA_TRANSP, 0);
                        lv_obj_set_style_border_width(spacer2, 0, 0);

                        // Transfer stats (updated by timer)
                        lv_obj_t *read_lbl = lv_label_create(msc_screen);
                        lv_label_set_text(read_lbl, LV_SYMBOL_UPLOAD "  0.0 MB read");
                        lv_obj_set_style_text_color(read_lbl, lv_color_hex(0x88CC88), 0);

                        lv_obj_t *write_lbl = lv_label_create(msc_screen);
                        lv_label_set_text(write_lbl, LV_SYMBOL_DOWNLOAD "  0.0 MB written");
                        lv_obj_set_style_text_color(write_lbl, lv_color_hex(0xCCCC88), 0);

                        // Spacer
                        lv_obj_t *spacer3 = lv_obj_create(msc_screen);
                        lv_obj_set_size(spacer3, 1, 4);
                        lv_obj_set_style_bg_opa(spacer3, LV_OPA_TRANSP, 0);
                        lv_obj_set_style_border_width(spacer3, 0, 0);

                        // Warning
                        lv_obj_t *warn_lbl = lv_label_create(msc_screen);
                        lv_label_set_text(warn_lbl, "ADS-B + Meshy logging paused");
                        lv_obj_set_style_text_color(warn_lbl, lv_color_hex(0x887744), 0);
                        lv_obj_set_style_text_font(warn_lbl, &lv_font_montserrat_14, 0);

                        // Spacer
                        lv_obj_t *spacer4 = lv_obj_create(msc_screen);
                        lv_obj_set_size(spacer4, 1, 16);
                        lv_obj_set_style_bg_opa(spacer4, LV_OPA_TRANSP, 0);
                        lv_obj_set_style_border_width(spacer4, 0, 0);

                        // Disconnect button
                        lv_obj_t *dc_btn = lv_btn_create(msc_screen);
                        lv_obj_set_size(dc_btn, 200, 48);
                        lv_obj_set_style_bg_color(dc_btn, lv_color_hex(0xCC4444), 0);
                        lv_obj_set_style_shadow_width(dc_btn, 0, 0);
                        lv_obj_set_style_radius(dc_btn, 8, 0);
                        ...
                        lv_obj_set_style_text_font(dc_lbl, &lv_font_montserrat_16, 0);
                        lv_obj_center(dc_lbl);

                        // Disconnect callback — exit MSC, delete screen
                        lv_obj_add_event_cb(dc_btn, [](lv_event_t *e3) {
                            if (lv_event_get_code(e3) != LV_EVENT_CLICKED) return;
                            lv_obj_t *msc_scr = (lv_obj_t *)lv_event_get_user_data(e3);
                            sd_msc_exit_fn();
                            // Delete the MSC overlay — returns to settings
                            lv_obj_del(msc_scr);
                        }, LV_EVENT_ALL, msc_screen);

                        // Stats update timer — runs every 500ms while MSC screen exists
                        struct MscTimerCtx { lv_obj_t *rd; lv_obj_t *wr; };
                        MscTimerCtx *tctx = (MscTimerCtx *)lv_malloc(sizeof(MscTimerCtx));
                        tctx->rd = read_lbl;
                        tctx->wr = write_lbl;

                        lv_timer_t *stats_timer = lv_timer_create([](lv_timer_t *timer) {
                            MscTimerCtx *c = (MscTimerCtx *)lv_timer_get_user_data(timer);
                            ...
                            uint64_t rd = 0, wr = 0;
                            sd_msc_get_stats_fn(&rd, &wr);
                            char buf[48];
                            snprintf(buf, sizeof(buf), LV_SYMBOL_UPLOAD "  %.1f MB read", rd / 1048576.0);
                            lv_label_set_text(c->rd, buf);
                            snprintf(buf, sizeof(buf), LV_SYMBOL_DOWNLOAD "  %.1f MB written", wr / 1048576.0);
                            lv_label_set_text(c->wr, buf);
                        }, 500, tctx);
                    }
                }, LV_EVENT_CLICKED, NULL);
        ...
        // Max aircraft
        {
            ...
            static const uint8_t ac_vals[] = {0, 16, 32, 48, 64, 96, 128};
            static const char *ac_labels[] = {"All", "16", "32", "48", "64", "96", "128"};
            int cur = 0;
            for (int i = 0; i < 7; i++) if (g_settings.scope_max_aircraft == ac_vals[i]) { cur = i; break; }
            lv_obj_t *btn = add_value_btn(row, ac_labels[cur]);
            lv_obj_add_event_cb(btn, [](lv_event_t *e) {
                if (lv_event_get_code(e) != LV_EVENT_CLICKED) return;
                static const uint8_t vals[] = {0, 16, 32, 48, 64, 96, 128};
                static const char *labels[] = {"All", "16", "32", "48", "64", "96", "128"};
                int cur = 0;
                for (int i = 0; i < 7; i++) if (g_settings.scope_max_aircraft == vals[i]) { cur = i; break; }
                cur = (cur + 1) % 7;
